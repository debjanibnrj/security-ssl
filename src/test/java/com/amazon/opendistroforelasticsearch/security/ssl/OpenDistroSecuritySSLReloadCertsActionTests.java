...
public class OpenDistroSecuritySSLReloadCertsActionTests extends AbstractUnitTest {
    ...
    private void initTestCluster(final String transportPemCertFilePath, final String transportPemKeyFilePath, final String httpPemCertFilePath, final String httpPemKeyFilePath, final boolean sslCertReload) throws Exception {
        final Settings settings = Settings.builder()
                .putList(SSLConfigConstants.OPENDISTRO_SECURITY_AUTHCZ_ADMIN_DN, "CN=kirk,OU=client,O=client,L=Test,C=DE")
                .putList(SSLConfigConstants.OPENDISTRO_SECURITY_NODES_DN, "C=DE,L=Test,O=Test,OU=SSL,CN=node-1.example.com")
                ...
                .put(SSLConfigConstants.OPENDISTRO_SECURITY_SSL_CERT_RELOAD_ENABLED, sslCertReload)
                .build();
        ...
        startES(settings,initTransportClientSettings);
    }
    ...
    public void testReloadTransportSSLCertsPass() throws Exception {
        ...
        expectedJsonResponse = new JSONObject();
        expectedJsonResponse.put("http_certificates_list", NODE_CERT_DETAILS);
        expectedJsonResponse.put("transport_certificates_list", NEW_NODE_CERT_DETAILS);
        Assert.assertEquals(expectedJsonResponse.toString(), certDetailsResponse);
    ...
    public void testReloadHttpSSLCertsPass() throws Exception {
        ...
        certDetailsResponse = executeSimpleRequest(GET_CERT_DETAILS_ENDPOINT);
        expectedJsonResponse = new JSONObject();
        expectedJsonResponse.put("http_certificates_list", NEW_NODE_CERT_DETAILS);
        expectedJsonResponse.put("transport_certificates_list", NODE_CERT_DETAILS);
        Assert.assertEquals(expectedJsonResponse.toString(), certDetailsResponse);
    }
